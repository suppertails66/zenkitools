...
TThingyTable::IdLengthPair
  ...
  return IdLengthPair(-1, -1);
}

TThingyTable::IdLengthPair TThingyTable::matchTableEntry(TStream& ifs) const {
  for (LengthCollection::const_iterator it = entriesByLength.cbegin();
       it != entriesByLength.cend();
       ++it) {
...
TThingyTable::IdLengthPair TThingyTable::matchId(TStream& ifs) const {
